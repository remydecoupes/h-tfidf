...
def h_tfidf_highest_scores(data: pd.DataFrame,
                           spatial_level: str,
                           temporal_level: str,
    ...
    :param top_n: The N top number of highest H-TFIDF scores per space and time windows
    :return: a pandas dataframe with 3 columns:
        ...
        - list_ranked_scores: list of H-TFIDF scores
    """
    # Check args
    ...
    if "text" not in data and "geo" not in data and "datetime" not in data:
        raise ValueError(f"Data are badly formatted. Column names needed: text, geo and datetime")
